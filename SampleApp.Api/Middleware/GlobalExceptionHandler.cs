...
namespace SampleApp.Api.Middleware
{
    public sealed class GlobalExceptionHandler
    {
        ...
        private async Task LogAndSendResponse(HttpContext context, Exception ex)
        {
            try
            {
                _logger.Log(LogLevel.Error, ex, $"GlobalExceptionHandlerMiddleware caught exception: {ex.Message}");
            }
            catch (Exception exInternal)
            {
                try
                {
                    _logger.Log(LogLevel.Error, exInternal, $"GlobalExceptionHandlerMiddleware internal exception when attempting to log an application exception ({ex.Message})");
                }
                catch
                {
